...
std::string TopicDifficultyProblem::getTopic() { return topic; }
int TopicDifficultyProblem::getDifficulty() { return difficulty; }


std::string TopicAuthorLengthProblem::getTopic() { return topic; }
std::string TopicAuthorLengthProblem::getAuthor(){return author;}
bool TopicAuthorLengthProblem::getIsLong(){return isLong;}

Problem::Problem(std::string rawProblem)
{
    std::string format = "^([\\s\\S]*)\\\\answer\\{([\\s\\S]*)\\}[\\s\\S]*";
    std::regex re(format);
    std::smatch match;
...
TopicDifficultyProblem::TopicDifficultyProblem(std::string rawProblem) : Problem(rawProblem)
{
...
TopicAuthorLengthProblem::TopicAuthorLengthProblem(std::string rawProblem) : Problem(rawProblem){
    ...
    topic = match.str(3);
    author = match.str(4);
    isLong = match.str(5) == "true" ? true : false;
}
...
std::vector<TopicDifficultyProblem> TopicDifficultyProblem::problemList(std::string filename)
{
    // Read problems from file
    std::ifstream file(filename);
    if (!file.is_open())
...
std::vector<TopicAuthorLengthProblem> TopicAuthorLengthProblem::problemList(std::string filename)
{
    // Read problems from file
    std::ifstream file(filename);
    if (!file.is_open())
